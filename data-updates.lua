-- Change the default any quality filter icon
data.raw["utility-sprites"]["default"].any_quality.filename = "__prismatic-quality__/graphics/icons/any-quality.png"
data.raw["virtual-signal"]["signal-any-quality"].icon = "__prismatic-quality__/graphics/icons/any-quality.png"

-- Add new quality
data.raw.quality["legendary"].next = "prismatic"
...
data:extend({
  {
    ...
    beacon_power_usage_multiplier = 1/12, -- 1/192
    mining_drill_resource_drain_multiplier = 1/12, -- 1/192
    science_pack_drain_multiplier = 90/100,
  }
})
table.insert(data.raw.technology["legendary-quality"].effects, { type = "unlock-quality", quality = "prismatic"})
...
data.raw["tips-and-tricks-item"]["quality-probabilities"].simulation =
{
  ...
  [[
    ...
    show_probabilities = function()
      ...
      for k, quality in pairs (quality_levels) do
        if quality.name ~= "prismatic" then
